...
// ✅ CAMBIO PRINCIPAL: Forzar regeneración dinámica en producción
export const dynamic = "force-dynamic";
export const revalidate = 0;

// ✅ CAMBIO PRINCIPAL: Desactivar generación estática en producción
export async function generateStaticParams(): Promise<{ slug: string }[]> {
  // ✅ CAMBIO: Retornar array vacío siempre para forzar generación dinámica
  return [];

  // ✅ COMENTADO: El código anterior que causaba el problema de cache
...
// Componente de página con mejor manejo de caché
export default async function EmpresaDetail(props: {
  ...
  console.log(
    `🔍 [EmpresaDetail] Cargando empresa con slug: "${slug}" - ${new Date().toISOString()}`
  );
  ...
  try {
    ...
    if (!empresa || !empresa.habilitado) {
      console.log(
        `❌ [EmpresaDetail] Empresa no encontrada o deshabilitada: ${slug}`
      );
      return notFound();
    }

    console.log(
      `✅ [EmpresaDetail] Empresa cargada: "${
        empresa.nombre
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        {/* ✅ AGREGADO: Metadata de debugging en desarrollo */}
              ...
              <div className="lg:col-span-1">
                <div className="relative aspect-square rounded-xl overflow-hidden bg-gray-100 shadow-lg">
                  <Image
                    ...
                    priority
                    // ✅ AGREGADO: Key para forzar recarga de imagen
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
                {imagenesSecundarias.map((url, i) => (
                  <div
                    key={`${url}-${i}-${Date.now()}`} // ✅ AGREGADO: Key única para forzar recarga
    ...
    console.error(`❌ [EmpresaDetail] Error cargando empresa ${slug}:`, error);
    return notFound();
  }
}
